...
if(argc!=3)
{
fprintf(stderr,"Usage:./client IP address_of_server port\n");
exit(1);
}
...
scanf("%d",&num_cols_1);
printf("Enter the value row by row one on each line:\n");
...
{
printf("MATRICES CANNOT BE MULTIPLE\n");
exit(1);
}
printf("Enter the values for row by row on each line: \n");
